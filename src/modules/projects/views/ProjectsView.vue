      ...
      <tbody>
        <tr v-for="(proyecto, i) in proyectosStore.proyectos" :key="i" class="hover">
          ...
          <td>
            ...
            <button @click="addTask(i)" class="btn btn-sm btn-primary ml-2">Agregar Tarea Automatica</button>
          </td>
          <td>
...
const addTask = (index) => {
  ...
  if (nuevoProyecto.tareas.length < 10) {
    ...
    nuevoProyecto.progreso = Math.min((nuevoProyecto.tareas.filter(t => t.completada).length) * 10, 100);
  } else {
